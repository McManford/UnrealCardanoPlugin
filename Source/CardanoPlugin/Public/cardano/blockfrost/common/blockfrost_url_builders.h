...
/**
 * \brief Sets a custom base URL for all Blockfrost API requests.
 *
 * This function allows overriding the default Blockfrost API URLs for all networks
 * with a custom URL. This is useful for scenarios where you want to route requests
 * through a proxy or a custom API gateway.
 *
 * \param[in] url       A pointer to the custom URL string.
 * \param[in] url_size  The size of the URL string in bytes.
 */
void cardano_blockfrost_set_custom_base_url(const char* url, size_t url_size);
