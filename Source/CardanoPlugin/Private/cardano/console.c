...
#define LOG_SEVERITY_ENV "LOG_SEVERITY"

// Use #define for string constants to avoid non-constant initializer issues
#define LOG_SEVERITY_INFO_STR   "info"
#define LOG_SEVERITY_ERROR_STR  "error"
#define LOG_SEVERITY_WARN_STR   "warn"
#define LOG_SEVERITY_DEBUG_STR  "debug"

// Declare log_severity globally but do not initialize it here
// Declare at global scope without initialization
static uint8_t log_severity;
static bool log_severity_initialized = false;
 ...
 * \return The log severity.
 */
uint8_t get_log_severity()
{
    if (!log_severity_initialized)
    {
        log_severity = LOG_SEVERITY_UNKNOWN; // Set default at runtime
        log_severity_initialized = true; // Mark as initialized

        const char* env_str = LOG_SEVERITY_WARN_STR; // Default log severity

#if !defined(UE_BUILD_SHIPPING)
        const char* temp_env = getenv(LOG_SEVERITY_ENV);
        if (temp_env != NULL) {
            env_str = temp_env;
        }
#endif
