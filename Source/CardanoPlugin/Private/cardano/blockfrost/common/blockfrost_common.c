...
/* INCLUDES ******************************************************************/
/* System headers */
/* This must come before curl.h to ensure time_t is defined */
#include <time.h>
#include <sys/types.h>  /* Additional definitions that might be needed */

/* Force time_t type to be visible to curl */
...
/* Include curl after all system headers and time_t definition */
#include <curl/curl.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
  ...
  cardano_buffer_t**                response_buffer)
{
  ...
  // Add these debug logging lines
  console_debug("Submitting transaction to URL: %s", url);
  console_debug("Transaction body size: %zu bytes", body_size);

  // Optional: print first few bytes of the transaction body in hex
  char hex_preview[100] = { 0 };
  for (size_t i = 0; i < (body_size > 50 ? 50 : body_size); i++) {
        char byte_hex[3];
        snprintf(byte_hex, sizeof(byte_hex), "%02x", body[i]);
        strcat(hex_preview, byte_hex);
  }
  console_debug("First bytes of transaction: %s", hex_preview);
  ...
  *response_code                = 0;
  ...
  // Add these lines near the start of the function
  curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
  curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
  curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
  curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, cardano_blockfrost_handle_response);
