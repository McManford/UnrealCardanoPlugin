...
/* STATIC FUNCTIONS **********************************************************/
// In blockfrost_url_builders.c - add these global variables
static char g_custom_url_base[256] = { 0 };
static bool g_use_custom_url = false;

// Add this function to set the custom URL
void 
cardano_blockfrost_set_custom_base_url(const char* url, size_t url_size)
{
    if (url && url_size > 0 && url_size < 256) {
        memset(g_custom_url_base, 0, sizeof(g_custom_url_base));
        memcpy(g_custom_url_base, url, url_size);
        g_custom_url_base[url_size] = '\0';
        g_use_custom_url = true;
    }
    else {
        g_use_custom_url = false;
    }
}

// Then modify the get_network_base_url function
const char* 
cardano_blockfrost_get_network_base_url(const cardano_network_magic_t network)
{
    // If a custom URL is set, use it regardless of network
    if (g_use_custom_url && g_custom_url_base[0] != '\0') {
        return g_custom_url_base;
    }

    // Otherwise use the default URLs based on network
    switch (network)
    {
